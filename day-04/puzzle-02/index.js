...
lineReader.on('close', () => {
  ...
  const greatest = sorted.reduce((agg, g) => {
    const maxMin = g.minutes.reduce((agg, m) => Math.max(agg, m), 0)
    return agg.max > maxMin ? agg : {
      id: g.id,
      max: maxMin
    }
  }, {id: 0, max: 0})
  const minute = sorted.find(s => s.id === greatest.id).minutes.indexOf(greatest.max)
  console.log(greatest.id, minute, greatest.id * minute)
